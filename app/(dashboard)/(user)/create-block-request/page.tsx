...
const activityOptions = getActivityOptions();
  ...
  const isDateInNextWeek = (dateString: string): boolean => {
    // Week boundaries are read in IST, matching isPastThursdayCutoff above.
    // Reading them in UTC rolled the week over 5h30m late, so between 00:00 and
    // 05:30 IST the wrong week was being compared.
    const ist = nowInIST();
    const today = new Date(
                ...
                <div key={block} className="flex flex-col gap-1 w-full">
                  ...
                  <div className="flex flex-row items-center gap-3 w-full">
                    ...
                    <button
                      ...
                      className="ml-2 px-3 py-1 rounded-lg bg-green-100 border border-green-400 text-green-800 text-sm font-semibold hover:bg-green-200 whitespace-nowrap"
                    >
                      + Add
                    </button>
                  </div>
