...
`;
    let someId = "";
    if (
        session?.user?.id !== "852e95b1-a568-4571-99e4-96bf7e02ba01" &&
    ...
    if (
        session?.user?.id !== "596aad5b-1e8b-42c1-ad1c-244d8774dedc" &&
    ...
    if (
        session?.user?.id !== "78a2a1d7-037a-4948-aa86-a33adf1a6596" &&
    ...
    const { data, isLoading, error } = useQuery({
        ...
        queryFn: async () => {
            try {
                const result = await managerService.getUserRequestsByManager(
                    1,
                    10000,
                    undefined,
                    undefined,
    ...
    const pendingRequests = (
        ...
        .filter(
            (r: UserRequest) =>
                r.status === "PENDING" && r.managerAcceptance === false
        )
    ...
    const rejectedRequest = (
        ...
        .filter(
            (r: UserRequest) =>
                (r.trdActionsNeeded === false && r.oheResponse !== "") ||
                (r.sigActionsNeeded === false && r.sigResponse !== "") ||
                r.overAllStatus === "return to applicant by optg" ||
                r.overAllStatus === "return to applicant by trd." ||
                r.overAllStatus === "return to applicant by s&t and trd." ||
                r.overAllStatus === "return to applicant by s&t."
        )
        .sort((a: UserRequest, b: UserRequest) => {
            // Priority sort: urgent blocks first
            const urgentA = a.corridorType === "Urgent Block" ? 0 : 1;
    ...
    const handleAccept = async (
        ...
        const isFridayAfterNoon =
            today === 5 && (hour > 12 || (hour === 12 && minute >= 0));
    ...
    const handleTimeChange = (
        field: "demandTimeFrom" | "demandTimeTo",
        part: "hour" | "minute",
        value: string
    ) => {
    ...
    const handleSaveEdit = async () => {
        ...
        try {
            ...
            if (data?.data?.requests) {
                ...
                const updatedRequests = [...data.data.requests].map(
                    (req: UserRequest) => {
    ...
    const handleBulkAccept = async () => {
        ...
        if (isFridayAfterNoon) {
            ...
            for (const id of selectedRequests) {
                ...
                if (!isUrgent && requestDate >= blockStart && requestDate <= blockEnd) {
                    alert(
                        `You cannot accept request ${request.divisionId || request.id
                        } (${formatDate(
                            request.date
                        )}) because it falls within the weekend period (Saturday to next Sunday) on Friday after 12 PM.`
                    );
                    return;
                }
            }
        ...
        if (
            !confirm(
                `Are you sure you want to forward ${selectedRequests.size} requests?`
            )
        )
            return;
        ...
        try {
            ...
            // Show success only once after all are done
            setShowSuccessModal(
                `${selectedRequests.size} requests forwarded successfully!`
            );
        } catch (error) {
            console.error("Error forwarding requests:", error);
            toast.error("Some requests failed. Check logs.");
        ...
        <div className="min-h-screen bg-[#FFFDF5]">
                ...
                <div className="mx-4 mt-6 flex items-center gap-4">
                    ...
                    {selectedRequests.size > 0 && (
                        <div className="flex gap-2">
                            ...
                            >
                                {bulkAcceptRequests.isPending
                                    ? "Processing..."
                                    : `Forward (${selectedRequests.size})`}
                            </button>
                            {/* <button
            ...
            {pendingRequests.length > 0 && (
                <div className="mx-4 mt-6 overflow-x-auto">
                    <div
                        className={`rounded-xl overflow-hidden border-2 border-black bg-[#F5E7B2] min-w-[700px] ${showRejectModal || showSuccessModal ? "invisible" : ""
                            }`}
                    >
                        <table className="w-full text-black text-base border-collapse">
                            <thead>
                                <tr className="bg-[#D6F3FF] text-black font-bold">
                                    ...
                                    <th className="border-2 border-black px-2 py-2 bg-[#D6F3FF]">
                                        Date
                                    </th>
                                    <th className="border-2 border-black px-2 py-2 bg-[#D6F3FF]">
                                        ID
                                    ...
                                    <th className="border-2 border-black px-2 py-2 sticky right-0 z-10 bg-[#D6F3FF] w-32">
                                        Actions
                                    </th>
                                </tr>
                            </thead>
                            <tbody>
                                {pendingRequests.map((request: UserRequest) => (
                                    <tr
                                        key={request.id}
                                        className={`hover:bg-[#FFF86B] text-black ${request.corridorType === "Urgent Block"
                                                ? "urgent-block-row"
                                                : "bg-white"
                                            }`}
                                    >
            ...
            {pendingRequests.length > 0 && (
                <div className="mx-4 mt-6 mb-8 flex justify-center gap-4">
                    <button
                        ...
                        className="bg-[#FFA07A] px-8 py-2 rounded-[50%] border-2 border-black font-bold"
                    >
                        Download
                    </button>
                    <Link
                        ...
                        <table className="w-full text-black text-base border-collapse">
                            ...
                            <tbody>
                                {rejectedRequest.map((request: UserRequest) => (
                                    <tr
                                        key={request.id}
                                        className="bg-white hover:bg-[#FFF86B] text-black"
                                    >
                                        <td className="border border-black px-2 py-1 text-center align-middle">
                                            {formatDate(request.date)}
                                        </td>
            ...
            {showSuccessModal && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
                    <div className="bg-white rounded-lg p-6 max-w-xs w-full flex flex-col items-center">
                        <div className="text-lg font-bold mb-4 text-center text-black">
                            {showSuccessModal}
                        </div>
                        <button
                            onClick={() => setShowSuccessModal(null)}
                    ...
                    <div className="bg-white rounded-lg p-6 max-w-md w-full">
                        ...
                        <div className="mb-4">
                            ...
                            <div className="flex items-center">
                                <select
                                    value={
                                        editFormData.demandTimeFrom
                                            ? editFormData.demandTimeFrom.split(":")[0]
                                            : ""
                                    }
                                    onChange={(e) =>
                                        handleTimeChange("demandTimeFrom", "hour", e.target.value)
                                    }
                                ...
                                <select
                                    value={
                                        editFormData.demandTimeFrom
                                            ? editFormData.demandTimeFrom.split(":")[1]
                                            : ""
                                    }
                                    onChange={(e) =>
                                        handleTimeChange("demandTimeFrom", "minute", e.target.value)
                                    }
                        ...
                        <div className="mb-4">
                            ...
                            <div className="flex items-center">
                                <select
                                    value={
                                        editFormData.demandTimeTo
                                            ? editFormData.demandTimeTo.split(":")[0]
                                            : ""
                                    }
                                    onChange={(e) =>
                                        handleTimeChange("demandTimeTo", "hour", e.target.value)
                                    }
                                    className="w-20 p-2 border border-gray-300 rounded-l text-black"
                                >
                                    <option value="">--</option>
                                ...
                                <select
                                    value={
                                        editFormData.demandTimeTo
                                            ? editFormData.demandTimeTo.split(":")[1]
                                            : ""
                                    }
                                    onChange={(e) =>
                                        handleTimeChange("demandTimeTo", "minute", e.target.value)
                                    }
                                    className="w-20 p-2 border border-gray-300 rounded-r text-black"
                                >
                                    <option value="">--</option>
