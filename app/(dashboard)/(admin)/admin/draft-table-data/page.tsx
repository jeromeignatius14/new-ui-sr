...
const getActivitiesForWorkType = (workTypeSelected: string): string[] => {
  ...
  return [
    "ALL",
    ...(Activity[workTypeSelected as keyof typeof Activity] || []),
  ];
};
...
const getActivitiesForDepartment = (dept: string): string[] => {
  ...
  return [
    "ALL",
    ...workTypes.flatMap(
      (type) => Activity[type as keyof typeof Activity] || []
    ),
  ];
};
...
useEffect(() => {
  ...
  const [deptFilter, setDeptFilter] = useState<string>(() => {
    ...
    return deptParam &&
      ["ENGG", "S&T", "TRD"].includes(decodeURIComponent(deptParam))
...
const [editFormData, setEditFormData] = useState<{
  ...
  const matchesWorkType = (request: UserRequest): boolean => {
    ...
    const deptWorkTypes =
      workType[request.selectedDepartment as keyof typeof workType] || [];
...
const { data, isLoading, error, refetch } = useQuery({
  ...
  const pendingRequests = (data?.data?.requests || []).filter(
    (request: UserRequest) => {
      if (!request.status || request.status.toUpperCase() !== "APPROVED")
        return false;
...
// Add this debug useEffect
  ...
  const urgentRequestsFiltered = pendingRequests
    .filter((r: UserRequest) => {
      ...
      const isUrgent =
        r.corridorType === "Urgent Block" || r.workType === "EMERGENCY";
      if (!isUrgent) return false;

      // Global filters
      if (deptFilter !== "ALL" && r.selectedDepartment !== deptFilter)
        return false;
      ...
      if (
        r.powerBlockRequired &&
        r.sntDisconnectionRequired &&
        r.enggDisconnectionsRequired
      ) {
        return (
          (r.trdActionsNeeded && r.sigActionsNeeded) ||
          (r.allTrdAcceptance === "ACCEPTED" &&
            r.allSntAcceptance === "ACCEPTED" &&
            r.allEnggAcceptance === "ACCEPTED")
        );
      }
  ...
  const corridorRequestsFiltered = pendingRequests
    .filter((r: UserRequest) => {
      ...
      const isCorridor =
        r.corridorType === "Corridor" || r.corridorType === "Corridor Block";
      ...
      const hasOtherLines = r.processedLineSections?.some(
        (section: any) => section.otherLines && section.otherLines.trim() !== ""
      );
      ...
      if (
        ...
        return (
          (r.trdActionsNeeded && r.sigActionsNeeded) ||
          (allTrdAcceptance && allSntAcceptance && allEnggAcceptance)
        );
      }
  ...
  const nonCorridorRequestsFiltered = pendingRequests
    .filter((r: UserRequest) => {
      ...
      const isNoncorridor =
        ...
        r.corridorType === "Non-Corridor Block";
    ...
    try {
      const UrgentRequestsData =
        requests
          ?.filter(
            (request: UserRequest) =>
              request.optimizeTimeFrom != null && request.optimizeTimeTo != null
          )
...
const formatTimeForInput = (dateString: string) => {
  ...
  const handleOptimize = async () => {
    ...
    const preData = isUrgentRequests
      ? urgentRequestsFiltered
      : [...corridorRequestsFiltered, ...nonCorridorRequestsFiltered];
    if (!preData) return;
    ...
    try {
      ...
      const requestsToOptimize = preData.filter((request: UserRequest) => {
  ...
  const shouldShowRejectButton = (request: UserRequest): boolean => {
    ...
    if (
      request.corridorType?.toLowerCase() === "Outside Corridor" ||
      (request.corridorType?.toLowerCase() === "non-corridor" &&
        demandHours > 1)
    )
      return true;
...
const TableSelectionControls = ({ 
  ...
  return (
    <div className="min-h-screen w-screen flex flex-col justify-between bg-white p-3 border border-black">
      ...
      <div>
        {/* Overall Title */}
        <h1 className="text-2xl font-bold text-center mb-6 text-[#13529e]">
          Requests With Me
        </h1>
        {showSuccess && (
          ...
          <div className="relative">
            ...
            {showWorkTypeDropdown && deptFilter !== "ALL" && (
              <div className="absolute top-full left-0 bg-white border border-black shadow-lg z-50 min-w-[150px]">
                {[
                  "ALL",
                  ...(workType[deptFilter as keyof typeof workType] || []),
                ].map((type) => (
                  <button
                    key={type}
                    onClick={() => {
              ...
              <div className="absolute top-full left-0 bg-white border border-black shadow-lg z-50 min-w-[200px] max-h-60 overflow-y-auto">
                {getActivitiesForWorkType(workTypeFilter).map((activity) => (
                  <button
                    ...
                    className={`block w-full text-left px-3 py-2 hover:bg-gray-100 text-black ${
                      activityFilter === activity ? "bg-blue-100" : ""
                    }`}
                  >
                    {activity}
                  </button>
      ...
      <div className="border-b-2 border-[#13529e] pb-3 flex justify-between items-center mb-4">
        ...
        <div className="relative group">
          ...
          <div className="absolute right-0 top-full mt-2 hidden group-hover:block bg-white border border-gray-200 rounded-lg shadow-xl p-4 z-50 min-w-[220px]">
            <h3 className="text-black font-bold text-sm mb-3 pb-2 border-b border-gray-200">
              Department Colors
            </h3>
  ...
  <div className="flex items-center">
    ...
    Select All
                ...
                <th className="border border-black p-2 text-left text-[24px] font-semibold text-black sticky top-0 bg-gray-100 z-10">
                  <ColumnHeader icon="work" title="Remark" />
                </th>
                <th className="border border-black p-2 text-left text-[24px] font-semibold text-black sticky top-0 bg-gray-100 z-10">
                ...
                .filter((request: UserRequest) => {
                  const requestDate =
                    typeof request.date === "string"
                      ...
                      : request.date;
                  return (
                    isSameDay(requestDate, selectedDate) &&
                    request.Draft === true
                  );
                })
                .map((request: UserRequest) => {
  ...
  <div className="flex items-center">
    ...
    Select All
                ...
                <th className="border border-black p-2 text-left text-[24px] font-semibold text-black sticky top-0 bg-gray-100 z-10">
                 ...
                 <th className="border border-black p-2 text-left text-[24px] font-semibold text-black sticky top-0 bg-gray-100 z-10">
                  <ColumnHeader icon="action" title="Remark" />
                </th>
                <th className="border border-black p-2 text-left text-[24px] font-semibold text-black sticky top-0 bg-gray-100 z-10">
                  <ColumnHeader icon="action" title="Actions" />
                </th>
            ...
            <tbody>
              ...
              {corridorRequestsFiltered
                .filter((request: UserRequest) => request.Draft === true)
                .sort(
                  (a: any, b: any) =>
                    new Date(a.demandTimeFrom).getTime() -
                    new Date(b.demandTimeFrom).getTime()
                )
                .map((request: UserRequest) => (
                  <tr
 key={`request-${request.id}-${request.date}`}
...
<td className="border border-black p-2 text-[24px]">
  ...
  <div className="flex items-center">
    ...
    Select All
            ...
            <tbody>
              {combinedRequestsFiltered.filter((request: UserRequest) => request.Draft === true&& request.corridorType !== "Urgent Block").length === 0 && (
                <tr>
                  <td
                    colSpan={12}
                    className="border border-black p-2 text-[24px] text-left"
              ...
              {combinedRequestsFiltered
                .filter((request: UserRequest) => request.Draft === true&& request.corridorType !== "Urgent Block")
                .sort(
                  (a: any, b: any) =>
                    new Date(a.demandTimeFrom).getTime() -
                    new Date(b.demandTimeFrom).getTime()
